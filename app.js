...
class HunterTraining {
    ...
    generateQuests() {
        ...
        if (questsList) {
            ...
            quests.forEach(quest => {
                const completeBtn = document.getElementById(`complete-${quest.id}`);
                if (completeBtn) {
    ...
    createQuestHTML(quest) {
        ...
        return `
            <div class="quest-item ${quest.type === 'non-negotiable' ? 'non-negotiable' : ''} ${isCompleted ? 'completed' : ''}">
                ...
                ${!isCompleted ? `
                    <button id="complete-${quest.id}" class="quest-complete" ${quest.progress < quest.target ? 'disabled' : ''}>
                        Complete
                    </button>
                ` : ''}
            </div>
    ...
    completeQuest(questId) {
        ...
        if (quest && quest.progress >= quest.target && !quest.completed) {
            this.playerData.xp += quest.reward.xp;
            this.playerData.gold += quest.reward.gold;
            ...
            this.showNotification(`Quest completed! +${quest.reward.xp} XP, +${quest.reward.gold} Gold`, 'success');
        }
    }

    getQuestById(questId) {
        // This would need to be implemented based on your quest storage
        return null;
    }
    ...
    startDungeon(tier) {
        if (this.playerData.ap < 10) {
            this.showNotification('Not enough AP! Complete quests to gain more Action Points.', 'error');
            return;
        }

        this.playerData.ap -= 10;
    ...
    startRaid(tier) {
        ...
        document.getElementById('raid-title').textContent = `Tier ${tier} Dungeon`;
        document.getElementById('boss-health').style.width = '100%';
        document.getElementById('boss-info').textContent = 
            `Tier ${tier} Boss - ${this.currentRaid.bossHealth}/${this.currentRaid.bossHealth} HP`;
    ...
    launchAttack() {
        ...
        document.getElementById('boss-info').textContent = 
            `Tier ${this.currentRaid.tier} Boss - ${this.currentRaid.bossHealth}/${this.currentRaid.maxBossHealth} HP`;
    ...
    logSteps() {
        ...
        if (steps > 0) {
            ...
            this.playerData.ap += Math.floor(steps / 1000); // 1 AP per 1000 steps
            ...
            this.showNotification(`Logged ${steps} steps! +${Math.floor(steps / 1000)} AP`, 'success');
        }
    }
    ...
    buyItem(itemId) {
        ...
        if (this.playerData.gold >= item.price) {
            ...
            if (item.type === 'consumable') {
                ...
                const slot = this.playerData.equipment[item.type];
                if (!slot) {
                    this.playerData.equipment[item.type] = item;
    ...
    sellDuplicates() {
        // Simple duplicate selling logic
    ...
    showEquipmentOptions(slotType) {
        // Show equipment management options
    ...
    setupDailyReset() {
        ...
        resetTime.setHours(2, 0, 0, 0); // 2 AM
        
        if (now > resetTime && this.playerData.lastReset !== new Date().toDateString()) {
            this.dailyReset();
        }
    ...
    dailyReset() {
        this.playerData.lastReset = new Date().toDateString();
    ...
    setupErrorHandling() {
        window.addEventListener('error', (e) => {
            ...
            this.showNotification('An error occurred. The game should still work.', 'error');
        });
...
window.closeResults = function() {
    document.getElementById('raid-results').classList.remove('active');
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        // Global functions for HTML onclick handlers
        window.startDungeon = (tier) => game.startDungeon(tier);
        
    } catch (error) {
        console.error('Failed to initialize game:', error);
        document.body.innerHTML = `
            <div style="padding: 20px; text-align: center; color: white; background: #1a1a2e; min-height: 100vh;">
                <h1>Hunter Training</h1>
                <p>Failed to load the game. Please refresh the page.</p>
                <p style="color: #e94560;">Error: ${error.message}</p>
                <button onclick="window.location.reload()" style="background: #e94560; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">
                    Reload Game
                </button>
            </div>
        `;
